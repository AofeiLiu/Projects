...
class SudokuPuzzle(Puzzle):
    ...
    A sudoku puzzle that may be solved, unsolved, or even unsolvable.
     ...
     === Attributes ===
    @type n: int
        The length of the play board that is square
    @type symbols: list[str]
        The play board of SudokuPuzzle
    @type symbol_set: set[str]
        The symbols that we used to solve the SudokuPuzzle
    """
    ...
    def fail_fast(self):
        """
        Return True if there is one open position where the symbols are already
        used in the same row, column or subsquare.
        ...
        if "*" not in symbols:
            ...
            for i in range(len(symbols)):
                if symbols[i] == "*":
                    allowed_symbols = (self._symbol_set - (self._row_set(i) |
                                                           self._column_set(i) | self._subsquare_set(i)))
